...
// ---------------------------------------------------------------------------
// POST /_test/wa-approval-escalation — admin-only dry-run.
// Runs the full escalation check (DB queries, pref gate, dedup) but skips the
// actual WA HTTP send. Writes send_whatsapp audit rows so actor can be asserted.
// Marks $app.store() as escalated so dedup tests work.
//
// Query params:
//   threshold_hours=<float>  (default 0 — all open requests qualify)
//
// Returns: { fired, open_requests, escalated, already_escalated }
// ---------------------------------------------------------------------------
routerAdd("POST", "/_test/wa-approval-escalation", function(c) {
  ...
  try {
    ...
    openRequests = $app.dao().findRecordsByFilter(
      "requests", filterStr, "-created", 200, 0, filterParams
    );
  } catch (e) {
  ...
  if (!openRequests || openRequests.length === 0) {
    return c.json(200, { fired: true, open_requests: 0, escalated: 0, already_escalated: 0 });
  }

  var admins = [];
  try {
    admins = $app.dao().findRecordsByFilter(
      "users", "role = 'admin' && phone != \"\"", "", 100, 0, {}
    );
  } catch (e) {
  ...
  if (!admins || admins.length === 0) {
    return c.json(200, { fired: true, open_requests: openRequests.length, escalated: 0, already_escalated: 0 });
  }
  ...
  for (var ri = 0; ri < openRequests.length; ri++) {
    ...
    for (var ai = 0; ai < admins.length; ai++) {
      ...
      try {
        ...
        var auditRec = new Record(auditCol);
        auditRec.set("collection_name", "messages");
        auditRec.set("record_id", "test-dryrun");
        auditRec.set("actor", admin.id);
        auditRec.set("action", "send_whatsapp");
        auditRec.set("changes", JSON.stringify({
          to: adminPhone,
        ...
        console.log("[wa_escalation/_test] audit write failed for admin " + admin.id + ":", auditErr);
      }
    }
