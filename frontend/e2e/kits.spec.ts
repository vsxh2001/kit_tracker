...
test.describe("Kits page — listing and search", () => {
  ...
  test("serial search filters results to matching kit", async ({ page }) => {
    ...
    await expect(page.locator("table tbody").getByRole("cell", { name: `${TS}-SEARCH`, exact: true })).toBeVisible();
  });
...
test.describe("Kit creation (admin)", () => {
  ...
  test("admin can create a new kit via dialog @smoke", async ({ page }) => {
    ...
    await expect(page.locator("table tbody").getByRole("cell", { name: SERIAL, exact: true })).toBeVisible({
...
test.describe("Kit delete via kits list row", () => {
  ...
  test("admin can delete a kit via row Delete button", async ({ page }) => {
    ...
    await expect(page.locator("table tbody").getByRole("cell", { name: DELETE_ROW_SERIAL, exact: true })).toBeVisible();
    ...
    await expect(page.locator("table tbody").getByRole("cell", { name: DELETE_ROW_SERIAL, exact: true })).not.toBeVisible({
      timeout: 5000,
