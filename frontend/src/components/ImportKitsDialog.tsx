...
import { importKitsCsv } from "../services/kits";
import type { ImportResult } from "../services/kits";
...
export function ImportKitsDialog({ open, onClose, onImported }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Import Kits from CSV</DialogTitle>
          <DialogDescription>
            First row should be header: serial,notes,is_active
          </DialogDescription>
        </DialogHeader>
            ...
            {result.skipped.length > 0 && (
              <ul className="ml-4 list-disc space-y-0.5 text-muted-foreground">
                {result.skipped.map((s) => (
                  <li key={`${s.row}-${s.serial}`}>
                    Row {s.row}: duplicate serial {s.serial}
                  </li>
                ))}
              </ul>
            )}
